...
exports.handler = async (event, context) => {
  try {
    const { data: program, error } = await supabase
      .from('pipeline_program')
      .select('*')
      .order('week_number', { ascending: true });

    if (error || !program || program.length === 0) {
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ weeks: defaultWeeks })
      };
    }
    ...
    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ weeks: Object.keys(weeks).length > 0 ? weeks : defaultWeeks })
    };
  } catch (e) {
    console.error('Program API error:', e.message);
    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ weeks: defaultWeeks })
